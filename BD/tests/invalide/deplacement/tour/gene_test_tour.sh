...
for p in 1 8 25 32
#on met les nums des différentes tours
do
for x in `seq 0 7`
do
for y in `seq 0 7`
do	
	...
	touch tour$p\_deplacement_$x\_$y.sql
	echo "-------------------------------------------------------------------------------
...
SPOOL tour$p\_deplacement_$x\_$y.lst
SET LINESIZE 100
SET PAGESIZE 50 

-- tour qui va autrement que une case tout droit
UPDATE PIECE
...
SPOOL OFF
EXIT;">>tour$p\_deplacement_$x\_$y.sql
done
done
done
