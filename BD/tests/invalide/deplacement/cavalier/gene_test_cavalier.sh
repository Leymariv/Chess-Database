...
for p in 2 7 26 31
#on met les nums des différents cavaliers
do
for x in `seq 0 7`
do
for y in `seq 0 7`
do	
	...
	touch cavalier$p\_deplacement_$x\_$y.sql
	echo "-------------------------------------------------------------------------------
...
SPOOL cavalier$p\_deplacement_$x\_$y.lst
SET LINESIZE 100
SET PAGESIZE 50 

-- cavalier qui va autrement que une case tout droit
UPDATE PIECE
...
SPOOL OFF
EXIT;">>cavalier$p\_deplacement_$x\_$y.sql
done
done
done
